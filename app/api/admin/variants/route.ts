...
import { listProductVariants, createProductVariant } from "@/lib/admin/services";
import { variantCreateSchema, searchQuerySchema } from "@/lib/admin/validation";
...
export async function GET(request: Request) {
  try {
    ...
    const result = await listProductVariants({ ...parsed.data, productId });
...
export async function POST(request: Request) {
  try {
    ...
    const variant = await createProductVariant(parsed.data);
